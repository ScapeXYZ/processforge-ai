...
function CreateWorkspace() {
  ...
  return <main className="relative min-h-screen overflow-hidden bg-background text-foreground"><WorkflowLogo className="pointer-events-none fixed left-1/2 top-1/2 z-0 size-[36rem] -translate-x-1/2 -translate-y-1/2 text-foreground opacity-[0.035]" /><div className="relative z-10"><WorkspaceHeader /><div className="grid min-h-[calc(100vh-4rem)] lg:grid-cols-[15rem_minmax(22rem,0.85fr)_minmax(28rem,1.3fr)]"><TemplateSidebar onSelect={chooseTemplate} /><SopForm values={values} onChange={setValues} onSubmit={generate} isGenerated={Boolean(sop)} isLoading={isLoading} error={error} /><SopPreview sop={sop} source={generationSource} onRegenerate={generate} onClear={clear} onSopChange={setSop} isLoading={isLoading} /></div></div></main>;
}
