...
set replay_fingerprint = payment_reference
where replay_fingerprint is null;

alter table public.agent_payments
  alter column replay_fingerprint set not null;

create unique index if not exists agent_payments_replay_fingerprint_unique
  on public.agent_payments(replay_fingerprint);
