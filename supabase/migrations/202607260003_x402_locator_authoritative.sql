...
begin
  if p_authorization_hash is null or p_authorization_hash = ''
    or (p_replay_key is null and (p_endpoint is null or p_endpoint = '')) then
  ...
  from public.agent_request_payloads arp
  where (p_replay_key is not null and arp.replay_key = p_replay_key)
    or (
      p_replay_key is null
  ...
  if v_row.replay_key is null
    or v_row.created_at < p_created_after
    or v_row.expires_at <= now()
    or (
      v_row.consumed_at is not null
      and v_row.consumed_authorization_hash <> p_authorization_hash
    )
    or (
      p_replay_key is not null
      and v_row.payer_address is not null
      and p_payer_address is not null
      and v_row.payer_address <> lower(p_payer_address)
    ) then
    return query select 'unavailable'::text, 0::bigint, null::text, null::text,
      null::text, null::text, null::text, null::timestamptz, null::timestamptz, null::text;
    return;
  ...
  return query select
    'found'::text, 1::bigint, v_row.replay_key, v_row.endpoint,
    v_row.payer_address, v_row.request_hash, v_row.body_text, v_row.expires_at,
    v_row.consumed_at, v_row.consumed_authorization_hash;
end;
$$;
