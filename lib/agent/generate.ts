...
export async function generateAgentSop(input: AgentSopRequest) {
  ...
  const response = await new OpenAI({ apiKey, timeout: 90_000, maxRetries: 1 }).responses.parse({ model: "gpt-5-mini", store: false, instructions: SOP_SYSTEM_PROMPT, input: buildSopPrompt(request), text: { format: zodTextFormat(sopSchema, "processforge_agent_sop") } });
