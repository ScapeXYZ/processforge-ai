...
export function inspectServerEnvironment(): { ready: boolean; errors: string[] } {
  ...
  const releaseChecksEnabled = parsed.data.NODE_ENV === "production" && (parsed.data.ENABLE_RELEASE_CHECK === "true" || process.env.VERCEL_ENV === "production");
  ...
  if (parsed.data.AGENT_PAID_GENERATION_ENABLED === "true") { if (parsed.data.OKX_X402_ENABLED !== "true") errors.push("OKX_X402_ENABLED must be true when paid generation is enabled"); if (parsed.data.OKX_X402_NETWORK !== "eip155:196") errors.push("Production x402 network must be eip155:196"); for (const key of ["OKX_X402_PAY_TO_ADDRESS", "OKX_X402_ASSET", "OKX_X402_ASSET_ADDRESS", "OKX_X402_ASSET_DECIMALS", "OKX_X402_PRICE", "OKX_X402_FACILITATOR_URL", "OKX_X402_API_KEY", "OKX_X402_SECRET_KEY", "OKX_X402_PASSPHRASE"] as const) if (parsed.data[key] === undefined || parsed.data[key] === "") errors.push(`Missing required x402 variable: ${key}`); }
