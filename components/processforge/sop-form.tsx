...
export function SopForm({ values, onChange, onSubmit, isGenerated }: { values: SopFormValues; onChange: (values: SopFormValues) => void; onSubmit: () => void; isGenerated: boolean }) {
  ...
  return (
    <section className="p-4 sm:p-6 lg:p-7">
      ...
      <form onSubmit={(event) => { event.preventDefault(); onSubmit(); }} className="space-y-5">
        ...
        <Button type="submit" size="lg" className="mt-2 h-11 w-full bg-emerald-500 text-emerald-950 hover:bg-emerald-400"><Sparkles className="size-4" />{isGenerated ? "Regenerate SOP" : "Generate SOP"}</Button>
        <p className="text-center text-[11px] text-muted-foreground">Generated locally for this prototype. No data leaves your browser.</p>
      </form>
    </section>
  );
}
