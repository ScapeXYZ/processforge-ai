...
const health = await fetch(`${baseUrl}/api/agent/health`);
...
const unpaid = await fetch(`${baseUrl}/api/agent/generate-sop`, { method: "POST", headers: { ...headers, "idempotency-key": paidIdempotencyKey }, body: JSON.stringify(body) });
...
const mockPaid = await fetch(`${baseUrl}/api/agent/generate-sop`, { method: "POST", headers: { ...headers, "idempotency-key": paidIdempotencyKey, "payment-signature": mockToken }, body: JSON.stringify(body) });
...
if (proof) {
  const paid = await fetch(`${baseUrl}/api/agent/generate-sop`, { method: "POST", headers: { ...headers, "idempotency-key": `${headers["idempotency-key"]}-paid`, "payment-signature": proof }, body: JSON.stringify(body) });
