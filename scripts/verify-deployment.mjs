...
if (metadata.body?.pricing?.enabled) { assert(invalid.response.status === 400, "Invalid request rejected before payment", `HTTP ${invalid.response.status}`); assert(!invalid.response.headers.get("payment-required"), "Invalid request payment challenge", "absent"); const payload = { title: "Deployment verification", description: "When a request arrives, Operations validates it, records evidence, and escalates exceptions.", industry: "Technology", department: "Operations", audience: "Operations team", output_format: "json" }; const unpaid = await request("/api/agent/generate-sop", { method: "POST", headers, body: JSON.stringify(payload) }); assert(unpaid.response.status === 402, "Unpaid production request", `HTTP ${unpaid.response.status}`); }
